...
这个例子响应`/seed/v1`路由，返回计算密集型处理的数据。
...
`火焰图（flamegraph）`是处理MySQL性能问题时发明的。它利用常规的剖析器/示踪器得到的文本产生可视化，允许快速准确地识别最频繁的代码路径, 可以更快的找出热点代码和调用堆栈之间的关系。恢复上面代码原来的样子，我们使用`0x(github@davidmarkclements)`这个工具进行分析，它可以通过分析`CPU profile`文件生成svg火焰图。
